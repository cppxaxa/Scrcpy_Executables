...
SCRCPY_dir = os.getcwd()
FFMPEG_bin = os.path.join(SCRCPY_dir, 'ffmpeg.exe')
ADB_bin = os.path.join(SCRCPY_dir, 'adb.exe')
...
class SCRCPY_client():
    ...
    def __del__(self):
        ...
        if self.fferrthrd is not None: self.fferrthrd.join()
        if self.ffinthrd is not None: self.ffinthrd.join()
        if self.ffoutthrd is not None: self.ffoutthrd.join()
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if frm is not None and isinstance(frm, (np.ndarray, np.generic)):
                frm = cv2.cvtColor(frm, cv2.COLOR_RGB2BGR)
                cv2.imshow("image", frm)
                ch = cv2.waitKey(1000//60)  # CAP 60FPS
